    ...
    <div class="container">
        ...
        <form action="" method="post">
            ...
            <?php if ($editType === 'task'): ?>
                ...
                <input type="date" name="deadline" id="deadline" value="<?= $data['deadline'] ?>" required>

                <!-- Menampilkan dan mengedit subtasks -->
                <h3>Subtugas:</h3>
                <?php while ($subtask = mysqli_fetch_assoc($subtasks)): ?>
                    <label for="subtask_<?= $subtask['subtaskid'] ?>"></label>
                    <input type="text" name="subtasks[<?= $subtask['subtaskid'] ?>]" id="subtask_<?= $subtask['subtaskid'] ?>" value="<?= htmlspecialchars($subtask['subtasklabel']) ?>">
                <?php endwhile; ?>
            <?php endif; ?>
